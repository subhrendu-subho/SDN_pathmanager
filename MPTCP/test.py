...
f=args.files
    ...
    rate = {}
    column = 2
data = [line.strip() for line in open(f, 'r')]
data
...
pat_iface
for k in sorted(rate.keys()):
    print k , pat_iface.match(k)
for k in sorted(rate.keys()):
    print k + " %s" %( pat_iface.match(k))
import re
re.search(pat_iface,k)
a=re.search(pat_iface,k)
a
pat_iface = re.compile(args.pat_iface)
pat_iface
re.match(pat_iface,k)
if(re.match(pat_iface,k)):
    print "x"
%cpaste
plt.title("TX rates")
plt.ylabel("Mbps")
plt.grid()
plt.legend()
plt.savefig("results/h2_bwm.png")
%save test.py
%history -f test.py
